...
test('shortcut validation rejects repeated modifiers', () => {
    expect(validateShortcut('Ctrl+Ctrl+S') === 'Shortcut repeats modifier Ctrl.', 'expected repeated modifiers to be rejected explicitly');
});
test('shortcut validation requires a modifier and a single primary key', () => {
    expect(validateShortcut('S') === 'Shortcut should include at least one modifier like CmdOrCtrl, Alt, or Shift.', 'expected shortcuts without modifiers to be rejected');
    expect(validateShortcut('Ctrl+A+B') === 'Shortcut can only include one non-modifier key.', 'expected shortcuts with multiple primary keys to be rejected');
});
...
test('settings payload validation rejects malformed response payloads', () => {
    ...
    expect(error !== null && error.message.includes('invalid shortcut'), 'expected the validator to fail fast on the first malformed field');
});
