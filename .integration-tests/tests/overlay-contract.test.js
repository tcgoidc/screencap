import { assertCaptureFrames, canBringAnnotationToFront, canDuplicateSelectedAnnotation, canEditSelectedTextAnnotation, canSendAnnotationBackward, reorderAnnotationBackward, reorderAnnotationToFront, shouldCancelTextEditorOnKeydown, shouldHideOverlayOnKeydown, } from '../src/overlay-contract.js';
const test = (name, run) => {
    try {
        run();
        console.log(`ok - ${name}`);
...
test('escape hides the overlay only when shortcuts are not blocked', () => {
    expect(shouldHideOverlayOnKeydown({ key: 'Escape', textEditorActive: false, keyboardShortcutBlocked: false }), 'expected Escape to hide the overlay when the overlay owns keyboard focus');
    expect(!shouldHideOverlayOnKeydown({ key: 'Escape', textEditorActive: false, keyboardShortcutBlocked: true }), 'expected Escape to stay local when another input control owns the shortcut');
    expect(!shouldHideOverlayOnKeydown({ key: 'Escape', textEditorActive: true, keyboardShortcutBlocked: false }), 'expected Escape not to hide the overlay while inline text editing is active');
});
